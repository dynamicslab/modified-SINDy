...
plt.subplot(3,1,2)
pp2=plt.plot(t,x[:,3],linewidth=0.5,color='k')
pp1=plt.scatter(t,xn[:,3],s=0.5,color='b')
plt.ylabel('y')
plt.grid(True)

plt.subplot(3,1,3)
pp3=plt.plot(t,x[:,5],linewidth=0.5,color='k')
pp1=plt.scatter(t,xn[:,5],s=0.5,color='b')
plt.ylabel('z')
plt.xlabel('t')
plt.tight_layout()
plt.grid(True)
...
pp4.plot3D(xn[:,0],xn[:,1],xn[:,2], color='red',linestyle='--',linewidth=8)
pp4.view_init(30, 0)
pp4.grid(False)
pp4.axis('off')
plt.savefig("Lorenz96_NoiseLevel_"+str(NoisePercentage)+".pdf")
...
pp4.plot3D((xn-NoiseID)[:,0], (xn-NoiseID)[:,1], (xn-NoiseID)[:,2], color='orange',linestyle='--',linewidth=8)
pp4.view_init(30, 0)
pp4.grid(False)
pp4.axis('off')
plt.savefig("Lorenz96_Denoised_NoiseLevel_"+str(NoisePercentage)+".pdf")
...
plt.savefig("Lorenz96_Pre_"+str(NoisePercentage)+".pdf")
...
print("\n Parameter Error:")
print(np.linalg.norm(Xi_base-Xi.numpy())/np.linalg.norm(Xi_base))

print("\n Correct structure?")
print(np.linalg.norm((Xi.numpy()!=0).astype(int)-(Xi_base!=0).astype(int))==0)
