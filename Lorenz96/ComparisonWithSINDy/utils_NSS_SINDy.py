...
def LibGPU(x):
    ...
    # Theta=tf.concat([tf.ones_like(z1),z1,z2,z3,z1**2,z1*z2,z1*z3,z2**2,z2*z3,z3**2],axis=1)                 
    
    # =============================================================================
    # The follwing code will build the library using "tf.concat" command, which will slow the computation.
    # =============================================================================
    ...
    for i in range(m):
        for j in range(i,m):
            for k in range(j,m):
                NewTerm=tf.gather(x,[i],axis=1)*tf.gather(x,[j],axis=1)*tf.gather(x,[k],axis=1)
                Theta=tf.concat((Theta,NewTerm),1)
    ...
    #         for k in range(j,m):
    #             for ij in range(k,m):
    #                 NewTerm=tf.gather(x,[i],axis=1)*tf.gather(x,[j],axis=1)*tf.gather(x,[k],axis=1)*tf.gather(x,[ij],axis=1)
    #                 Theta=tf.concat((Theta,NewTerm),1)
    ...
    #                 for ij in range(k,m):
    #                     for ik in range(ij,m):
    #                         NewTerm=tf.gather(x,[i],axis=1)*tf.gather(x,[j],axis=1)*tf.gather(x,[k],axis=1)*tf.gather(x,[ij],axis=1)*tf.gather(x,[ik],axis=1)
    #                         Theta=tf.concat((Theta,NewTerm),1)                       
