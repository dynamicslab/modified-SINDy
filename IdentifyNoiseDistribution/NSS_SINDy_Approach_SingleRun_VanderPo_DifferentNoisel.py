...
elif NoiseType=='gamma':
    Noise=np.hstack([NoiseMag[i]*np.random.gamma(1,1,(dataLen,1)) for i in range(stateVar)])
elif NoiseType=='uniform':
    #Noise=np.hstack([np.random.uniform(-np.sqrt(3*NoiseMag[i]),np.sqrt(3*NoiseMag[i]),(dataLen,1)) for i in range(stateVar)])
    Noise=np.hstack([NoiseMag[i]*np.random.uniform(-1,1,(dataLen,1))/2 for i in range(stateVar)])
elif NoiseType=='dweibull':
    # To check the mean and variance use
    # mean, var, skew, kurt = dweibull.stats(1, moments='mvsk')
    Noise=Noise=np.zeros((dataLen,stateVar))
    # Assign noise
    for i in range(stateVar):
        Noise[:,i]=NoiseMag[i]*dweibull.rvs(2.07,size=dataLen)
elif NoiseType=='rayleigh':
    Noise=np.hstack([NoiseMag[i]*np.random.rayleigh(1,(dataLen,1)) for i in range(stateVar)])
...
# First plot the noise: true v.s. identified
StartIndex=1000 # Choose how many noise data point you would like to plot
EndIndex=1500
...
plt.subplot(2,1,1)
plt.grid(True)
pp6=plt.hist(Noise[q+1:-q-1,0], bins = n_bins, color = 'blue', alpha=0.9,edgecolor = 'black',density=True)
pp6=plt.hist(NoiseID[q+1:-q-1,0], bins = n_bins, color = 'orange', alpha=0.75,edgecolor = 'black',density=True)
...
plt.subplot(2,1,2)
plt.grid(True)
pp6=plt.hist(Noise[q+1:-q-1,1], bins = n_bins, color = 'blue', alpha=0.9,edgecolor = 'black',density=True)
pp6=plt.hist(NoiseID[q+1:-q-1,1], bins = n_bins, color = 'orange', alpha=0.75,edgecolor = 'black',density=True)
plt.ylabel('Frequency')
plt.xlabel('Noise:y')
axes = plt.gca()
#axes.set_xlim([-4*max(NoiseMag),4*max(NoiseMag)])
#axes.set_ylim([0,0.65])
#plt.savefig('Result_NSS_SwipeNoiseLevel\Plots\Percent'+str(NoisePercentage)+'_Distribution.pdf')

#%%
plt.figure()
pp1=plt.plot(x[q+1:-q-1,0],x[q+1:-q-1,1],linewidth=lw,color='k',linestyle='-')
pp1=plt.plot(xn[q+1:-q-1,0]-NoiseID[q+1:-q-1,0],xn[q+1:-q-1,1]-NoiseID[q+1:-q-1,1],linewidth=lw,color='orange',linestyle='--')
plt.ylabel('x')
plt.grid(False)
plt.axis('off')
#plt.savefig("SummaryFig\VanderPol_Denoised_NoiseLevel_"+str(NoisePercentage)+".pdf")
